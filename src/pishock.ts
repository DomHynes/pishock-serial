...
type Events = {
    info: [info: PiShockDeviceInfo],
    shockers: [shockers: Shocker[]]
}
...
export class SerialPiShock extends EventEmitter<Events> {
    constructor(
        ...
        port.on('error', err => this.logger.error({ err: err.toString() }, "porterror"))
    }
        ...
        for (const char of newData) {
            ...
            if (char === '\n' && this.line.slice(-2).join('') === '\r\n') {
                ...
                if (str.startsWith('TERMINALINFO: ')) {
                    const infoBlock: PiShockDeviceInfo = JSON.parse(str.replace('TERMINALINFO: ', ''))
